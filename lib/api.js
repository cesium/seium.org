...
export async function uploadStaffCV(formData) {
  const response = await API.patch(`/api/staff/upload_cv`, formData);

  return response.data;
}

export async function getStaffCV() {
  const response = await API.get(`/api/staff/cv`);
